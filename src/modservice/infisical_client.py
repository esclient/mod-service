...
class InfisicalSecretManager:
    def __init__(
        ...
        self.client: Any = InfisicalSDKClient(host=host, token=server_token)
    ...
    def get_secret(self, secret_name: str) -> str:
        ...
        return cast(str, secret.secretValue)
...
def create_infisical_manager() -> InfisicalSecretManager:
    ...
    if not server_token:
        raise ValueError(
            "INFISICAL_SERVER_TOKEN environment variable is required"
        )
    if not project_id:
        raise ValueError(
            "INFISICAL_PROJECT_ID environment variable is required"
        )
    if not host:
        raise ValueError("INFISICAL_HOST environment variable is required")
    if not environment_slug:
        raise ValueError(
            "INFISICAL_ENVIRONMENT environment variable is required"
        )
    if not secret_path:
        raise ValueError(
            "INFISICAL_SECRET_PATH environment variable is required"
        )
